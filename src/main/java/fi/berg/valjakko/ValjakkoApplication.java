...
public class ValjakkoApplication {
	...
	public CommandLineRunner valjakko (DogRepository repository, KennelRepository krepository, UserRepository urepository) {
		return (args) -> {
			...
			repository.save(new Dog("Vili", "uros","lahjakas", "12345678", krepository.findByName("Vauhtiviikarit").get(0)));
			repository.save(new Dog("Vilja", "naaras","lahjakas", "23456789", krepository.findByName("Punainen kuu").get(0)));
			repository.save(new Dog("Seppo", "uros","lahjakas", "34567890", krepository.findByName("Seinäjoen salamat").get(0)));
